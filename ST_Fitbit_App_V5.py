...
def main():
    ...
    select_mode_form.subheader("日付選択")
    ...
    st.write(select_day)
    st.session_state.Goal_STEP = st.sidebar.number_input('目標歩数を入力してください')
    st.session_state.Goal_HR = st.sidebar.number_input('目標心拍数を入力してください')

    if 'mode' not in st.session_state:
        st.session_state.mode = 0
        st.session_state.flag_num = 0
    #st.write(type(select_day))
    # 日付選択ボタンをクリックしたら
    if day_select_first_submit:
        st.session_state.mode = 1# data取得
        st.session_state.flag_num = 1# グラフ描画
    if st.session_state.mode == 1:
        ...
        @st.cache_data
        def make_time_series(stats,name,date=select_day,time_name='time'):
            time=[]
            param=[]
            for data in stats:
        ...
        @st.cache_data
        def get_weekly_HR(url_list,headers):
            dict_list=[]
            for url in url_list:
                res=request(session.get, url, headers=headers)
                dict_list.append(res.json())
            return dict_list
        ...
        def getdata(date: str = "today", period: str = "1d"):
            ...
            if period=="1d":
                ...
                date_list=pd.date_range(end=date, periods=7).to_series().dt.strftime("%Y-%m-%d")
                # パラメタを埋め込んでエンドポイント生成
                timeseries_heart_url_list=[
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[0]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[1]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[2]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[3]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[4]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[5]}/{'1d'}/{'15min'}.json",
                    f"https://api.fitbit.com/1/user/-/activities/heart/date/{date_list[6]}/{'1d'}/{'15min'}.json"
                ]
            ...
            if period=='1w':
                heart_res_list = get_weekly_HR(timeseries_heart_url_list,headers)
            else:
                heart_res_list=[]
            ...
            return heart_res_list,heart_res.json(),cal_res.json(),stp_res.json(),weight_res.json(),act_res.json()

        @st.cache_data
        def make_weekly_HR_dataframe(dicts):
            df_list=[]
            for _dict in dicts:
                date=_dict['activities-heart'][0]['dateTime']
                d_HR_df=make_time_series(_dict['activities-heart-intraday']['dataset'],'HR',date=date)
                temp=d_HR_df.reset_index()
                temp['date']=temp['time']
                temp['time']=temp['time'].dt.time
                time_range=pd.date_range(start=date, periods=96, freq="15min")
                time_df=time_range.to_frame(name='date')
                time_df['time']=time_df['date'].dt.time
                d_HR_df=pd.merge(temp[['time','HR']],time_df[['time']],on='time',how='right').sort_values(by=['time'])
                d_HR_df=d_HR_df.rename(columns={'HR': date})
                d_HR_df.drop('time', axis=1,inplace=True)
                df_list.append(d_HR_df)
        ...
        ################################ データ取得と変換  ##################################################
            ...
            _,d_heart,d_cal,d_stp,d_body,act= getdata(date=select_day,period='1d')
            summary=act['summary']
            ...
            w_heart_series,w_heart,w_cal,w_stp,w_body,_act= getdata(date=select_day,period='1w')
            weekly_body=make_body_summary(w_body)
            w_HR_df_list=make_weekly_HR_dataframe(w_heart_series)
            ...
            _,m_heart,m_cal,m_stp,m_body,_act= getdata(date=select_day,period='1m')
        ...
        st.session_state.mode =0
        st.session_state.day_summary=day_summary
        st.session_state.d_df=d_df
        st.session_state.w_df=w_df
        st.session_state.m_df=m_df
        st.session_state.w_HR_df_list=w_HR_df_list
        
    else:
        pass
    if st.session_state.flag_num == 1:
        ################################################################# 描画部分
        ...
        if stock=='1日単位':
            day_summary=st.session_state.day_summary
            restHR=str(day_summary['restHeartRate'].values[0])
            try:
                meanHR=str(int(d_df['HR'].mean()))
            except:
                meanHR='--'
            Calorie=str(day_summary['CaloriesOut'].values[0])
            Step=str(day_summary['Steps'].values[0])
            d_df=st.session_state.d_df
            #Goal_HR=100
            # 実行
            high_loc=d_df['HR'].values>=st.session_state.Goal_HR
            high_dt=d_df[high_loc].index
            high_HR=d_df['HR'][high_loc]
            time_high_HR=str(len(high_HR))
            
            line_Goal_HR=[st.session_state.Goal_HR]*len(d_df.index)
            ...
            ax1.plot(d_df.index,line_Goal_HR,color=c3,linewidth=line_width+3,linestyle="dotted",label='目標値')
            ax1.axhspan(st.session_state.Goal_HR, d_df['HR'].max(), color = c3, alpha=0.2)
            ...
            ax1.set_title('平均心拍数 '+meanHR+'bpm'+' 目標心拍数達成時間 '+time_high_HR+' 分', fontsize=fontsize,fontname="MS Gothic")
            ...
            w_df=st.session_state.w_df
            w_HR_df_list=st.session_state.w_HR_df_list
            line_Goal_STEP=[st.session_state.Goal_STEP]*len(w_df.index)
            ...
            high_loc=w_df['STEP'].values>=st.session_state.Goal_STEP
            high_dt=w_df[high_loc].index
            high_Step=w_df['STEP'][high_loc]
            date_high_Step=str(len(high_Step))
            ...
            data=w_HR_df_list.copy().set_index('time')
            # Nan値を除外
            week=[data.iloc[:,i].values[~np.isnan(data.iloc[:,i].values)] for i in range(7)]
            # 何もない場合、平均心拍数
            for i in range(len(week)):
                if week[i].size==0:
                    week[i]=np.array(int(data.mean().mean()))
            labels=[label for label in data.columns]

            try:
                average_HR=str(int(data.mean().mean()))
                max_HR=str(int(data.mean().max()))
                min_HR=str(int(data.mean().min()))
            except:
                average_HR='--'
                max_HR='--'
                min_HR='--'
            vio=ax1.violinplot(week, showmedians=True)
            ax1.set_xticklabels(labels)
            ax1.plot(np.arange(7)+1,data.mean().values,color=c3,linewidth=2,linestyle='dashed',label='平均心拍数')
            # 返値のキーにbodiesを利用
            ...
            ax3.plot(w_df.index,line_Goal_STEP,color=c3,linewidth=line_width+3,linestyle="dotted",label='目標値')
            ax3.axhspan(st.session_state.Goal_STEP, w_df['STEP'].max(), color = c3, alpha=0.2)
            ...
            ax2.set_title('平均消費カロリー '+average_Cal+ 'kcal'+' 最小 '+min_Cal+' 最大 '+max_Cal,\
                        fontsize=fontsize,fontname="MS Gothic")
            ax3.set_title('平均歩数 '+average_Step+ '歩'+' 最小 '+min_Step+' 最大 '+max_Step+' 目標達成日数 '+date_high_Step+' 日',\
                fontsize=fontsize,fontname="MS Gothic")
            ...
            line_Goal_STEP=[st.session_state.Goal_STEP]*len(m_df.index)

            average_HR=str(int(m_df.HR.mean()))
            ...
            high_loc=m_df['STEP'].values>=st.session_state.Goal_STEP
            high_dt=m_df[high_loc].index
            high_Step=m_df['STEP'][high_loc]
            date_high_Step=str(len(high_Step))
            ...
            ax3.axhspan(st.session_state.Goal_STEP, m_df['STEP'].max(), color = c3, alpha=0.2)
            ax3.plot(m_df.index,line_Goal_STEP,color=c3,linewidth=line_width+3,linestyle="dotted",label='目標値')
            ...
            ax1.set_title('安静時平均心拍数 '+average_HR+'bpm '+' 最小 '+min_HR+' 最大 '+max_HR,\
                        fontsize=fontsize,fontname="MS Gothic")
            ax2.set_title('平均消費カロリー '+average_Cal+ 'kcal'+' 最小 '+min_Cal+' 最大 '+max_Cal,\
                        fontsize=fontsize,fontname="MS Gothic")
            ax3.set_title('平均歩数 '+average_Step+ '歩'+' 最小 '+min_Step+' 最大 '+max_Step+' 目標達成日数 '+date_high_Step+' 日',\
        ...
        filename=st.session_state.USER+select_day+'_'+stock
